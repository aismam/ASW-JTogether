...
const MODIFIED_ACTIVITY_MESSAGE = name => `L'attività ${name} è stata modificata`
...
module.exports = notificationsController => {
    ...
    async function modifyActivity(req, res, next) {
        activityModel.modifyActivity(req.body, req.user)
            .then(modifiedActivity => {
                manageNotification(modifiedActivity.participants, notificationsController, modifiedActivity, MODIFIED_ACTIVITY_MESSAGE)
    ...
    async function deleteActivity(req, res, next) {
        Promise.all([userModel.deleteActivity(req.user, req.body), activityModel.deleteActivity(req.body, req.user)])
            .then(values => {
                manageNotification(values[USERS_TO_BE_NOTIFIED], notificationsController, values[DELETED_ACTIVITY], DELETION_SUCCESSFUL_MESSAGE)
    ...
    function manageNotification(users, notificationsController, activity, message) {
        const onlineUsers = activity.participants.filter(u => notificationsController.userIsOnline(u))
        const offlineUsers = activity.participants.filter(u => !notificationsController.userIsOnline(u))
        onlineUsers.forEach(u => notificationsController.notify(u, message(activity.name)))
        offlineUsers.forEach(u => userModel.createNotification(u, message(activity.name), activity.name, activity.creator_username))
    }
}
